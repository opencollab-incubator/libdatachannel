...
#include <array>
#include <atomic>
// Test operations inside assertions must also run in Release builds.
#ifdef NDEBUG
#undef NDEBUG
#endif
#include <cassert>
...
static void respond(Socket &server, int family, uint16_t expectedPort, uint16_t mappedPort) {
	...
	assert(poll(&pending, 1, 2000) == 1);
	std::array<unsigned char, 2048> request{};
	sockaddr_storage source{};
	socklen_t len = sizeof(source);
	auto size = recvfrom(server.fd, reinterpret_cast<char *>(request.data()), static_cast<int>(request.size()),
	                     0, reinterpret_cast<sockaddr *>(&source), &len);
	...
	assert(sendto(server.fd, reinterpret_cast<const char *>(response.data()), static_cast<int>(response.size()),
	              0, reinterpret_cast<sockaddr *>(&source), len) == static_cast<int>(response.size()));
}
...
static void testMonitor(int family) {
	...
	// The monitor must retain the socket before its first response, without a refresh wait.
	...
	assert(initial.mappingRevision == 0 && initial.mappedAddress[0] == 0); // owned copy
	...
	std::thread reader([&] {
		while (!stop) {
			...
			if (status == RTC_ERR_SUCCESS)
				assert(current.mappingRevision == 1 && current.mappedPort == 40000 && current.successfulResponses == 1);
			++reads;
	...
	stop = true;
	reader.join();
	rtcStunBinding closed{};
	assert(rtcGetStunUdpMuxBinding(monitor, 0, &closed) == RTC_ERR_INVALID);
	...
	std::cout << "STUN C/C++ monitor IPv" << (family == AF_INET ? 4 : 6)
	          << ": shared socket, owned snapshots, concurrent close PASS\n";
}

TestResult test_stun_udp_mux_monitor() {
#ifdef _WIN32
	WSADATA data;
	assert(WSAStartup(MAKEWORD(2, 2), &data) == 0);
#endif
	testMonitor(AF_INET);
	testMonitor(AF_INET6);
#ifdef _WIN32
	WSACleanup();
#endif
